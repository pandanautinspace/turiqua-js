	...
	if (state==1){
		...
		// make a bullet when you click
			...
			if(state==1 && canvas_x <=900){
				clicked = true;
				bulletlist.push(new bullet(hero.x,hero.y,hero.x - (herosetx - canvas_x),hero.y - (herosety - canvas_y),5));
			}
...
var update = function (modifier) {
	...
	if(clicked===true){
		...
		if((bulletlist[i].x+5)<xx || (bulletlist[i].x-5)>xx){
			bulletlist[i].x+=cX;
			bulletlist[i].y+=cY;
		} else if((bulletlist[i].y+5)<xy || (bulletlist[i].y-5)>xy) {
			bulletlist[i].x+=cX;
			bulletlist[i].y+=cY;
		}else if(bulletlist[i].x==xx || bulletlist[i].y==xy){
