...
//IF THIS DOESNT WOIRKAEFJSDFKKJADKSJDFADALSDKFJALKFDJALKSJDLKADS
...
/**
 * kne is a set of kne's extra stuff which isn't to useful.
 */
var kne = {
    /**
     ...
     */
    inArray: function (/*{String} val the value to search for*/val, array){return array.indexOf(val) == -1 ? false: true;}
}
    ...
    if (state == 2) {
		...
		if (stuffNum == 0 && hero.name.length <= 7){
            if (16 in keysDown){
                ...
                theKey in kne.arrayFromTo(65,90) ? hero.name += String.fromCharCode(theKey): kne.doNothing();
            } else{
                var theKey = whichKeys[0]+32;
                theKey in kne.arrayFromTo(97,122) ? hero.name += String.fromCharCode(theKey): kne.doNothing();
            }
	if (65 in keysDown){
		hero.name += 'a';
		stuffNum += 1;
