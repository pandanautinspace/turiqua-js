    ...
    if (state == 2) {
		...
		if (stuffNum == 0 && hero.name.length <= 7){
            if (16 in keysDown){
                ...
                kne.inArray(theKey,kne.arrayFromTo(65,91)) ? hero.name += String.fromCharCode(theKey): kne.doNothing();
            } else{
                var theKey = whichKeys[0]+32;
                kne.inArray(theKey,kne.arrayFromTo(97,123)) ? hero.name += String.fromCharCode(theKey): kne.doNothing();
            }
	if (65 in keysDown){
		hero.name += 'a';
		stuffNum += 1;
